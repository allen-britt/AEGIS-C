"""
AEGIS‑C Target Discovery Client
=================================

Standalone offensive discovery tool for external targets
"""
...
class TargetDiscoveryClient:
    ...
    def _scan_ports(self, host: str, port_range: str, timeout: int):
        ...
        for port in range(start_port, min(end_port + 1, start_port + 100)):  # Limit to first 100 ports for speed
            try:
...
if __name__ == "__main__":
    ...
    # Target configuration (acknowledged but not hardcoded)
    target_host = "192.168.1.209"  # This can be made configurable via CLI or UI
    
    print("=" * 60)
    print("🎯 AEGIS‑C Target Discovery - Offensive Reconnaissance")
    print("=" * 60)
    
    # Run discovery
    results = discovery.scan_target(target_host, port_range="1-500", timeout=2)
    
    print("\n" + "=" * 60)
    print("📊 DISCOVERY RESULTS")
    print("=" * 60)
    print(f"Target: {results['target_host']}")
    print(f"Open Ports: {len(results['open_ports'])}")
    print(f"AI Indicators: {len(results['ai_indicators'])}")
    ...
    # Export findings
    print("\n" + "=" * 60)
    print("💾 EXPORTING FINDINGS FOR DEFENSIVE INTEGRATION")
    print("=" * 60)
    
    export_data = discovery.export_findings("json")
    
    # Save to file
    with open(f"discovery_findings_{target_host.replace('.', '_')}.json", "w") as f:
        f.write(export_data)
    
    print(f"Findings exported to: discovery_findings_{target_host.replace('.', '_')}.json")
    print("\n✅ Discovery complete. Use findings to harden AEGIS‑C defenses.")